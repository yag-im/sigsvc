...
class TestAuth:
    async def test_valid_auth_token(self):
        res = await h.auth(Headers({"cookie": f"{h.AUTH_TOKEN_COOKIE_NAME}={VALID_AUTH_TOKEN}"}))
        assert res is None

    # TODO: handle expiration
    # async def test_valid_session_cookie(self):
    #    res = await h.auth(Headers({"cookie": f"{h.FLASK_SESSION_COOKIE_NAME}={VALID_FLASK_SESSION_COOKIE}"}))
    #    assert res is None

    async def test_invalid_auth_token(self):
        res = await h.auth(Headers({"cookie": f"{h.AUTH_TOKEN_COOKIE_NAME}=dcba"}))
        assert res == (http.HTTPStatus.UNAUTHORIZED, [], b"Invalid auth token\n")

    async def test_invalid_session_cookie(self):
        res = await h.auth(Headers({"cookie": f"{h.FLASK_SESSION_COOKIE_NAME}=.abc"}))
        assert res == (http.HTTPStatus.UNAUTHORIZED, [], b"Invalid auth token\n")
